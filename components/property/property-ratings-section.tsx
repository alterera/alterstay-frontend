...
import { getRatingLabel } from "@/lib/property-enrichment";
import { cn } from "@/lib/utils";
import type {
  PropertyGuestReviewDetail,
...
const RATING_ITEMS: {
  key: keyof PropertyReviewSummary["breakdown"];
  label: string;
}[] = [
...
export function PropertyRatingsSection({
  ...
  const { guestRating, reviewCount, breakdown } = reviewSummary;
  const displayedReviews = reviews.slice(0, 3);
  const hasReviews = reviewCount > 0 && guestRating != null;
          ...
          <div className="grid items-center gap-6 lg:grid-cols-2 lg:gap-8">
            ...
            <div className="space-y-2">
              {RATING_ITEMS.map((item) => {
                const value = breakdown[item.key] ?? guestRating;
                return (
                  <div
                    key={item.key}
